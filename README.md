...
1. Users launch a new Spark session via an API request to the proxy server. The proxy server authenticates the user through a variety of means, and launches a new Spark session in the configured remote cluster. This remote session is secured with a randomly generated token.
...
The server is impleneted as a raw hyper server that intercepts requests and then:
...
- Authentication was previously provided within the ServerIterceptor, but now is built-in to Spark 4.0 via the `spark.connect.authenticate.token` config.
